...
| `confidence-prompt` | Appended to all fix prompts | Ask Claude to output `REPRODUCED`, `VERIFIED`, and `CONFIDENCE_PERCENT` markers |
...
### Summary
Added null check in UserService.findById() to handle missing records gracefully instead of throwing NullPointerException.

### What failed
NullPointerException thrown when UserService.findById() receives an ID that doesn't exist in the database.

### Changes
<details>
<summary>src/UserService.ts</summary>
...
<details>
<summary>test/UserService.test.ts</summary>
...
The summary, error description, and fix title are generated by Claude via the `summary-prompt` markers (`FIX_TITLE`, `FIX_DESCRIPTION`, `FIX_ERROR`). The "What failed" section is omitted when no `FIX_ERROR` marker is found. Diffs are split per file with collapsible sections. If the total diff exceeds 50,000 characters, it is truncated with: "_Diff truncated. Accept the fix to see full changes, or use `/ci-assistant explain` for a detailed walkthrough._"
